...
class Rock(Projectile):
    def __init__(self):
        Projectile.__init__(self, "Rocks.png", 2)
...
class Octorok(Enemy):
    ...
    def move(self, frame):
        if self.step == 25:
            self.speed = 0
            # only fire a rock 1 in 3 times
            shouldFire = random.randint(0,2)
            if shouldFire == 2:
                aRock = self.shoot()
                self.step += 1
                return aRock
    ...
    def shoot(self):
        aProjectile = Rock()
        aProjectile.rect.x = self.rect.x
        aProjectile.rect.y = self.rect.y
        aProjectile.orientation = self.orientation
        showSprite(aProjectile)
        return aProjectile
class Gibdos(Enemy):
    def __init__(self):
